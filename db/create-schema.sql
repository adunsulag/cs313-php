...
BEGIN
	...
	CASE t_name
		...
		WHEN 'Appointment' THEN
			select concat(t.id,'(start=',start_date,', end=',end_date,')') into entity from Appointment t where t.id = t_id;
		ELSE
	...
	VALUES(t_name, t_id, 'SELECT', CONCAT(t_name, ' ', entity, ' was viewed.'), current_setting('act_log.user')::integer, NOW());
END
$BODY$
